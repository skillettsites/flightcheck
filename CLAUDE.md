...
## Design (Airbnb-clean, 19 Sep evening)
Soft white background, Plus Jakarta Sans, charcoal text, coral `#FF385C` primary buttons only. Rounded cards with soft shadow. Homepage: sparse header, short friendly headline, floating check card, three steps, then evidence / sample / pricing. Quiet ticker below the steps, not above the fold. No mustard CTAs, no condensed scream headlines, no dark ops-terminal chrome. Class names `.pass` / `.board` / `.ledger` / `.tape` remain; they now look like soft cards. `className="plain"` on any `<details>` that is not an FAQ. No emoji. No Dave / DaveKnowsAI in the UI.
