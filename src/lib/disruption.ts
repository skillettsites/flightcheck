...
export async function airportBySlug(slug: string): Promise<CoveredAirportRow | null> {
  const rows = await sbSelect<CoveredAirportRow>("fcc_airports_covered", `slug=eq.${encodeURIComponent(slug)}&limit=1`);
  return rows[0] ?? null;
}

export async function airportByIcaoRow(icao: string): Promise<CoveredAirportRow | null> {
  const rows = await sbSelect<CoveredAirportRow>("fcc_airports_covered", `icao=eq.${encodeURIComponent(icao)}&limit=1`);
  return rows[0] ?? null;
}

export async function allCoveredAirports(): Promise<CoveredAirportRow[]> {
  return sbSelect<CoveredAirportRow>("fcc_airports_covered", `select=*&order=state.asc,name.asc&limit=1000`);
}

export async function worstDays(icao: string, limit = 30): Promise<AirportDayDelay[]> {
  return sbRpc<AirportDayDelay>("fcc_airport_worst_days", { p_icao: icao, p_limit: limit });
}

export async function recentDays(icao: string, limit = 60): Promise<AirportDayDelay[]> {
  return sbSelect<AirportDayDelay>("fcc_apt_delay_daily", `icao=eq.${encodeURIComponent(icao)}&order=day.desc&limit=${limit}`);
}

export async function dayRow(icao: string, day: string): Promise<AirportDayDelay | null> {
  const rows = await sbSelect<AirportDayDelay>("fcc_apt_delay_daily", `icao=eq.${encodeURIComponent(icao)}&day=eq.${day}&limit=1`);
  return rows[0] ?? null;
}

export async function dayAcrossAirports(day: string, limit = 12): Promise<AirportDayDelay[]> {
  return sbRpc<AirportDayDelay>("fcc_day_across_airports", { p_day: day, p_limit: limit });
}
...
export async function worstDaysSince(sinceDay: string, minDelay: number, limit = 40): Promise<AirportDayDelay[]> {
  return sbSelect<AirportDayDelay>("fcc_apt_delay_daily", `day=gte.${sinceDay}&delay_min=gte.${minDelay}&order=delay_min.desc&limit=${limit}`);
}
